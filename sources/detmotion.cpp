...
DetMotion::DetMotion(QWidget *parent) :
    QMainWindow(parent)
   ...
   while (waitKey(5)<0)
    {
        ...
        Point vect(maxLoc.x-templateRect.x);

        // Draw green rectangle and the translation vector

        rectangle(frame2,workingRect,Scalar(0, 255, 0),2);
        Point p(workingCenter.x,workingCenter.y+vect.y);

        workingCenter.x=workingCenter.x+vect.x;
